...
# Placeholders:
#   {participants} / {platekeys} / {disease_types} – must be IN (...) strings
#   {like_clauses} – an OR-joined list of LIKE clauses, e.g. "diag_all LIKE '%C34%' OR diag_all LIKE '%D05%'"
# Fill placeholders via str.format(...) and/or BaseLoader.run_batched().
...
QUERIES = {
	...
	# HES: apc/ae/op (diag_all LIKE ...), include date column
	"hes_apc_diag_all_like": """
		SELECT DISTINCT participant_id, diag_all, disdate AS date
		FROM hes_apc
		WHERE participant_id IN {participants}
		  AND ( {like_clauses} )
	""",
	"hes_ae_diag_all_like": """
		SELECT DISTINCT participant_id, diag_all, arrivaldate AS date
		FROM hes_ae
		WHERE participant_id IN {participants}
		  AND ( {like_clauses} )
	""",
	"hes_op_diag_all_like": """
		SELECT DISTINCT participant_id, diag_all, apptdate AS date
		FROM hes_op
		WHERE participant_id IN {participants}
		  AND ( {like_clauses} )
	""",

	# Mortality (ICD10 multiple cause)
	"mortality_icd10_like": """
		...
		SELECT participant_id, diag_all, date
		FROM mort
		WHERE ( {like_clauses} )
	""",

	# Mental health – 2014-2016 style tables
	"mhmd_event_icd10_like": r"""
		WITH mhmd AS (
			SELECT DISTINCT participant_id,
		...
		SELECT participant_id, diag_mhmdprim, diag_mhmdsec
		FROM mhmd
		WHERE REGEXP_REPLACE(diag_mhmdprim, '\.', '') LIKE {like_clauses}
		   OR REGEXP_REPLACE(diag_mhmdsec,  '\.', '') LIKE {like_clauses}
	""",
	"mhldds_event_icd10_like": r"""
		WITH mhldds AS (
			SELECT DISTINCT participant_id,
				   primarydiagnosis   AS diag_mhmdprim,
				   secondarydiagnosis AS diag_mhmdsec
			FROM mhldds_event
			WHERE participant_id IN {participants}
		)
		SELECT participant_id, diag_mhmdprim, diag_mhmdsec
	...
	# MHSDS family – 6 specific tables
	"mhsds_medical_history_previous_diagnosis_icd10": r"""
		WITH t AS (
			SELECT DISTINCT participant_id,
				   prevdiag AS diag,
		...
		WHERE (diagscheme IN ('2','02'))
		  AND ( REGEXP_REPLACE(diag, '\.', '') LIKE {like_clauses} )
	""",
	"mhsds_provisional_diagnosis_icd10": r"""
		WITH t AS (
			SELECT DISTINCT participant_id,
				   provdiag AS diag,
				   diagschemeinuse AS diagscheme
		...
		WHERE (diagscheme IN ('2','02'))
		  AND ( REGEXP_REPLACE(diag, '\.', '') LIKE {like_clauses} )
	""",
	"mhsds_primary_diagnosis_icd10": r"""
		WITH t AS (
			SELECT DISTINCT participant_id,
				   primdiag AS diag,
				   diagschemeinuse AS diagscheme
		...
		WHERE (diagscheme IN ('2','02'))
		  AND ( REGEXP_REPLACE(diag, '\.', '') LIKE {like_clauses} )
	""",
	"mhsds_secondary_diagnosis_icd10": r"""
		WITH t AS (
			SELECT DISTINCT participant_id,
				   secdiag AS diag,
				   diagschemeinuse AS diagscheme
	...
	"mhsds_care_activity_icd10": r"""
		...
		WHERE (diagscheme IN ('1','01'))
		  AND ( REGEXP_REPLACE(diag, '\.', '') LIKE {like_clauses} )
	""",
	"mhsds_indirect_activity_icd10": r"""
		WITH t AS (
			SELECT DISTINCT participant_id,
				   codefind AS diag,
				   findschemeinuse AS diagscheme
	...
	# Cancer-source ICD-10 pattern across tables
	"cancer_icd10_from_table": r"""
		SELECT DISTINCT participant_id, {code_col} AS code_raw
		FROM {table}
		WHERE participant_id IN {participants}
		  AND ( REGEXP_REPLACE({code_col}, '\.', '') LIKE {like_clauses} )
	""",
